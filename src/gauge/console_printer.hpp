...
public:
    ...
    console_printer() : printer("console")
    {
    }
    ...
    void end()
    {
        ...
        double time = static_cast<double>(
            std::chrono::duration_cast<std::chrono::microseconds>(m_total_stop -
                                                                  m_total_start)
                .count());
    ...
    void benchmark_result(const benchmark& info, const tables::table& results)
    {
        ...
        std::cout << std::fixed << console::textgreen << "[ RUN      ]"
                  << console::textdefault << " " << info.testcase_name() << "."
                  << info.benchmark_name() << " (" << results.rows()
        ...
        double time = static_cast<double>(
            std::chrono::duration_cast<std::chrono::microseconds>(
                m_benchmark_stop - m_benchmark_start)
                .count());
...
private:
    template <class T>
    bool print_column(const std::string& column, const std::string& unit,
                      const tables::table& results)
    {
        ...
        statistics res = calculate_statistics(values.cbegin(), values.cend());
        ...
        std::cout << column << " " << std::endl
                  << console::textgreen << "[          ] "
                  << console::textdefault << "   Average: " << res.m_mean << " "
                  << unit << std::endl;
    ...
    void print(const std::string& name, const std::string& unit, double value,
               double mean)
    {
        std::cout << console::textgreen << "[          ] "
                  << console::textdefault << std::setw(11) << name << " "
                  << value << " " << unit << " ("
                  << (value < mean ? console::textred : console::textgreen)
                  << (value < mean ? "" : "+") << value - mean << " " << unit
                  << " / " << (value < mean ? "" : "+")
